    ...
    <div className="min-h-screen bg-background pb-24">
      <div className="sticky top-0 z-10 glass-effect glass-border">
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          <div>
            <h1 className="text-lg font-semibold tracking-tight text-foreground">{t("results.title", language)}</h1>
            <p className="text-xs text-muted-foreground">{t("results.subtitle", language)}</p>
          </div>
        </div>
      </div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}
        ...
        <p className="text-[11px] font-semibold text-foreground mb-2.5">{t("results.legend", language)}</p>
        <div className="grid grid-cols-5 gap-1.5">
          {[
            { label: t("results.excellent", language), cls: "bg-score-emerald" },
            { label: t("results.good", language), cls: "bg-score-mint" },
            { label: t("results.standard", language), cls: "bg-score-slate" },
            { label: t("results.poor", language), cls: "bg-score-amber" },
            { label: t("results.critical", language), cls: "bg-score-ruby" },
          ].map((item) => (
        ...
        <p className="text-[9px] text-muted-foreground mt-2.5 leading-relaxed">
          {t("results.intensityNote", language)}
        </p>
      </motion.div>
